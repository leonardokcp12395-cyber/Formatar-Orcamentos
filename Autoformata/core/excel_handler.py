...
class OrcamentoEngine:
    ...
    def gerar_excel_final(self, linhas_aprovadas, modelo_path, mapa_colunas, info):
        ...
        try:
            ok, save_path = self._preparar_arquivo(modelo_path)
            if not ok: return False, save_path, {}
            ...
            # Salva com retry caso tenha bloqueado no meio do processo
            try:
                self.wb_out.save(save_path)
            except PermissionError:
                return False, f"ALERTA: O arquivo '{save_path}' foi aberto durante o processo.\nFeche-o e tente novamente.", {}
    ...
    def _preparar_arquivo(self, modelo_path):
        """
        Cria cópia do modelo. Se o arquivo destino estiver aberto (bloqueado),
        cria automaticamente uma versão _v1, _v2, etc.
        """
        ...
        while tentativas < 20:
            if tentativas == 0:
                ...
                sufixo = f"_v{tentativas}"
            
            nome_final = f"{nome_base}{sufixo}.xlsx"
        ...
        if tentativas >= 20:
            return False, f"Muitos arquivos '{nome_base}' abertos! Feche o Excel."
        ...
        try:
            ...
            return False, f"Erro ao preparar arquivo: {str(e)}"
    ...
    def _processar_cabecalho(self):
        ...
        self._write_cell('A18', f"DATA DE ELABORAÇÃO DO ORÇAMENTO: {info.get('data', '')} (VALIDADE: 45 DIAS)", bold=True)
        ...
        self._write_cell('C20', f"Nº {info.get('num_orcamento', '')}", bold=True)
        self._write_cell('D20', f"DATA DE EMISSÃO: {info.get('data_emissao', '')}", bold=True)
        self._write_cell('D21', f"DATA DE INÍCIO: {info.get('data_inicio', '')}", bold=True)
        self._write_cell('E20', f"PRAZO: {info.get('prazo', '')}", bold=True)
        self._write_cell('G20', f"EMPENHO: {info.get('empenho', '')}", bold=True)
    ...
    def _processar_rodape(self, current_row, start_row):
        ...
        self._safe_write(r1, 8, f"=SUBTOTAL(9, H{start_row}:H{ultima_linha_dados})")
        
        # O ROUNDDOWN no Excel força o truncamento em 2 casas decimais sem arredondar para cima
        self._safe_write(r2, 8, f"=ROUNDDOWN(H{r1}*{bdi_val}, 2)")
        self._safe_write(r3, 8, f"=H{r1}+H{r2}")
        
        # Aproveitei para truncar o cálculo do Fator de Desconto também, para manter a consistência
        self._safe_write(r4, 8, f"=ROUNDDOWN(H{r3}*{fator_desconto}, 2)") 
        self._safe_write(r5, 8, f"=H{r3}-H{r4}")
    ...
    def _ajustar_altura_linha(self, row, desc_txt, altura_base):
        ...
        altura_final = altura_base if linhas_estimadas == 1 else max(altura_base, linhas_estimadas * 15)
    ...
    def _aplicar_precisao(self, valor, modo):
        ...
        if modo == "TRUNC":
            ...
            return val_float
        elif modo == "ROUND": return round(val_float, 2)
            ...
            for col in range(1, self.ws_src.max_column + 1):
                ...
                if isinstance(cell_tgt, MergedCell):
                     for m in self.ws_out.merged_cells.ranges:
                         if cell_tgt.coordinate in m: self.ws_out.unmerge_cells(str(m)); break
                     cell_tgt = self.ws_out.cell(tgt_row, col)
        ...
        try:
            ...
            for i, atual in enumerate(mapa):
                ...
                if idx_fim > i:
                    ...
                    self._safe_write(row_pai, 8, f"=SUBTOTAL(9, H{r_ini}:H{r_fim})", self.FMT_CONTABIL)
                    self.ws_out.cell(row_pai, 8).font = Font(bold=True)
