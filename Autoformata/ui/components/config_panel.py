...
class ConfigPanel(ctk.CTkScrollableFrame):
    def __init__(self, master, main_window, **kwargs):
        ...
        self.main_window.combo_modelos = ctk.CTkComboBox(f_l_inner, width=250, command=self.main_window._ao_trocar_modelo)
        self.main_window.combo_modelos.pack(side="left", padx=5)
        ...
        self.main_window.ent_line = ctk.CTkEntry(f_l_inner, width=50)
        self.main_window.ent_line.insert(0, "4")
        self.main_window.ent_line.pack(side="left")
        ...
        self.main_window.combo_bdi = ctk.CTkComboBox(fin_grid, width=250, values=["28,82% (SUP - Desc 0,19)", "35,18% (PRUMO - Desc 0,0601)", "0,00%"])
        self.main_window.combo_bdi.grid(row=0, column=1, padx=5, pady=5, sticky="w")
        self.main_window.inputs_refs["bdi_combo"] = self.main_window.combo_bdi

        ctk.CTkLabel(fin_grid, text="Método de Cálculo:").grid(row=1, column=0, padx=5, pady=5, sticky="w")
        self.main_window.combo_metodo_calc = ctk.CTkComboBox(fin_grid, width=250, values=["Cortar Casas (Padrão - Ignora resto)", "Arredondar (2 Casas - Matemático)", "Exato (Sem tratamento - Excel)"])
        self.main_window.combo_metodo_calc.grid(row=1, column=1, padx=5, pady=5, sticky="w")
        self.main_window.combo_metodo_calc.set("Cortar Casas (Padrão - Ignora resto)")

        ctk.CTkLabel(fin_grid, text="Altura Linha (Px):").grid(row=0, column=2, padx=(20, 5), pady=5, sticky="w")
        self.main_window.ent_altura = ctk.CTkEntry(fin_grid, width=100)
        self.main_window.ent_altura.insert(0, "24.75")
        self.main_window.ent_altura.grid(row=0, column=3, padx=5, pady=5, sticky="w")

        self.main_window.chk_pdf = ctk.CTkCheckBox(fin_grid, text="Gerar PDF automaticamente após Excel", text_color="lime")
        self.main_window.chk_pdf.grid(row=1, column=2, columnspan=2, padx=(20, 5), pady=5, sticky="w")
        ...
        for i, camp in enumerate(campos):
            ...
            self.main_window.combos_map[camp] = cb
